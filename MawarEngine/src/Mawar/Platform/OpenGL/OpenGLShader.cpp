...
namespace Mawar
{
	static GLenum ShaderTypeFromString(const std::string& type)
	{
		...
		M_CORE_ASSERT(false, "Unknown shader type!");
		return 0;
	}
	...
	std::string OpenGLShader::ReadFile(const std::string& filepath)
	{
		...
		{
			M_CORE_ERROR("Cannot open file: {0}", filepath);
		}
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			M_CORE_ASSERT(eol != std::string::npos, "Syntax error!");
			...
			size_t lastPos = pos == std::string::npos ? source.size() - 1 : nextLinePos;
			shaderSources[ShaderTypeFromString(type)] = source.substr(nextLinePos, pos - lastPos);
		}
	...
	void OpenGLShader::Compile(const std::unordered_map<GLenum, std::string>& shaderSources)
	{
		...
		for (auto&& [type, shaderSource] : shaderSources)
		{
			...
			if (isCompiled == GL_FALSE)
			{
				...
				M_CORE_ERROR("{0}", infoLog.data());
				M_CORE_ASSERT(false, "Shader compiling is failed.");
		...
		if (isLinked == GL_FALSE)
		{
			...
			// Use the infoLog as you see fit.
			M_CORE_ERROR("{0}", infoLog.data());
			M_CORE_ASSERT(false, "Shader program is not linked correctly.");
